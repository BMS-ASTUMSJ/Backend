...
const submitAssignment = async (req, res) => {
  try {
    ...
    // ============================================================
    // RESUBMISSION AFTER MENTOR REQUEST
    // ============================================================
...
const updateSubmission = async (req, res) => {
  try {
    ...
    // Only pending submissions can be normally updated
    ...
    // Keep the deadline restriction for normal updates
    if (assignment.deadline && new Date() > new Date(assignment.deadline)) {
      return res.status(400).json({
        success: false,
        message: "The assignment deadline has passed.",
    ...
    // IMPORTANT:
    // Updating does NOT reset the status.
    // It stays Pending.
